...
1. 专治 skill 文档内容臃肿、信息分配不均衡的问题
2. 审查 skill 文档是否符合规范,判断哪些文件需要拆分
3. 参考 LLM Wiki 式索引,验证每个文件都能被路由检索到
4. 每条规则都援引 Anthropic 官方定义与最佳实践背书
5. 验证 skill 放进全局环境后能否被正常识别、调用
...
1. **API key** —— 验证在干净环境里跑,和当前上下文隔开,结果只反映修复本身。
2. **你的模型的上下文长度** —— skill-doctor 把所有 skill 的 description 加起来,看会不会让你正在测的那个触发失败。
